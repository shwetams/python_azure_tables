...
def _update_storage_header(request,use_json=True):
...
def _update_storage_table_header(request, content_type='application/atom+xml',use_json=True):
    ...
    if(use_json):
        request.headers.append(('Accept','application/json;odata=nometadata'))
